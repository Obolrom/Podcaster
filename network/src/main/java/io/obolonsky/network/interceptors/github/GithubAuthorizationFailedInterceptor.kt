...
class GithubAuthorizationFailedInterceptor(
    ...
    private fun refreshToken(): Boolean {
        ...
        val tokenRefreshed = runBlocking {
            runCatching {
                val refreshRequest = GithubAppAuth.getRefreshTokenRequest(tokenStorage.refreshToken.orEmpty())
                GithubAppAuth.performTokenRequestSuspend(authorizationService, refreshRequest)
            }
                .getOrNull()
                ?.let { tokens ->
